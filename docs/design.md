...
reflects the current, decided state. The dated record of when and why
each direction was decided lives in [decisions.md](decisions.md); when
the two disagree, this document wins and the divergence is a bug to fix
in the same change that finds it. Both documents must stay publishable:
no private fixtures, live captures, customer or partner identifiers,
...
- No AMI 1.x protocol support: supported versions are AMI 2.0.0 and
  newer (Asterisk 12+). Older sessions are out of scope rather than
  refused; the banner remains diagnostic-only.
